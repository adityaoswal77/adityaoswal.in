...
- **Homepage Hero backdrop**: dark = `FishSchool` (`components/background/FishSchool.tsx`) — an underwater scene run through the same Bayer dither/pixelate post-pass the old `Dither` used, so the water and the fish alike resolve into dots. Fish are drawn per-pixel in the fragment shader (curved-spine body + hinged caudal fin + eye), the school parts around the pointer, and count/scale/canvas dpr step down under 640px. `Dither.tsx` is kept but no longer mounted anywhere — it is the one-line revert path. Light = `WarpBackground` (`components/ui/warp-background.tsx`) — a 3D perspective grid tunnel with beams falling toward the vanishing point, `gridColor="var(--border)"` (lavender), beams in the light-persona palette (tangerine pop first, then pastels). Rendered only when `isLight` (`app/page.tsx` Hero backdrop `div`); dark keeps the fish. The CTA/heading accent/badge dot still use the one-off blue (`#2554EB`), not tangerine. `.marker-highlight` and `PastelField.tsx` remain **unused/orphaned** — don't re-wire them into Hero without asking. (Earlier the light hero was a flat `var(--background)` with no animated backdrop; the warp backdrop superseded that on user request.)
- **Light hero edge fade**: the warp grid's near-plane creates a hard rectangular "window" edge in the middle of the viewport (an artifact of the CSS 3D perspective technique, not a real container border). Two `isLight`-only overlay `div`s on top of it soften this: a `backdrop-blur-md` ring masked with `[mask-image:radial-gradient(ellipse_65%_65%_at_center,transparent_35%,black_100%)]` (blur only kicks in away from center) plus a matching `radial-gradient(transparent 40%, var(--background) 100%)` vignette. Both need `-webkit-mask-image` alongside `mask-image` for Safari. Tune by eye against the actual rendered corner — the visible "window" size doesn't map cleanly to simple percent-of-viewport math because of the perspective transform.
